...
bool test_tonality_on_cmajor_scale() {
    ...
    // Frequencies for C major scale (C4..B4)
    std::vector<float> freqs = {261.63f, 293.66f, 329.63f, 349.23f, 392.00f, 440.00f, 493.88f};
    // Create buffer and sum sines
    AudioBuffer buf(1, static_cast<size_t>(dur * sr), sr);
    float* d = buf.getChannel(0);
    for (auto f : freqs) {
        AudioBuffer s = make_sine(f, sr, dur, 0.2f);
    ...
    return (key == "C" && mode == "major" && conf > 0.7);
}
